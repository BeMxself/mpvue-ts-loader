...
function compileMPScript (script, mpOptioins, moduleId) {
  ...
  if (script.lang === 'ts') { // 处理ts
    ...
    // 如果不是extend Vue class写法的组件重新用babel处理
    if (!metadata.isClassComp) {
        result = babel.transform(tsc(scriptContent), babelOptions);
...
function compileMP (content, mpOptioins) {
  ...
  if (resourcePath.toLowerCase().endsWith('.ts')) {
    content = tsc(content)
  }
  const babelrc = getBabelrc(mpOptioins.globalBabelrc)
